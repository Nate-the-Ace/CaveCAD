...
| 7 -- Surface Data | `<sha7>` | Run **Surface Data** on a drawing with no location set. Does the refusal tell a beginner exactly what to do next, in words they have already met in the menu? Then run it with both boxes ticked on an anchored cave and check imagery and contours both still land as they used to. |
