...
| 4 -- Callout | `<sha4>` | Place one typed callout and one floor-elevation callout. Is the two-option dialog answerable by a student who has not met the word LRUD? The dialog is modal, so it is the one part no automated check can reach. |
