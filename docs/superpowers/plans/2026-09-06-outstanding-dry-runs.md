...
| 8 -- Survey Notebook | `<sha8>` | Correct a trip's date from inside the Notebook (**... > Edit this trip...**). Was it findable without knowing it used to be its own tool? Check the trip count is unchanged afterwards -- the fork-a-duplicate bug is the whole reason this code exists. |
