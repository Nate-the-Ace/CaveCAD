...
| 3 -- Repair Drawing | `<sha3>` | Open a cave that needs repair and run **Repair Drawing** once, reading only the dialog. Could a student tell from the three checkbox labels alone what each does and whether it is safe? (The dialog is modal, so it is the one part of this tool no automated check can reach.) |
