...
// ---------------------------------------------------------------------
// THE PANELS A BAY NEEDS OPEN.
//
// A section bay is a place you TRACE: the scan is staged, the ghost
// outline is there to scale it against, and the actual work is Feature
// Trace and the Symbol Palette. Those are exactly the two panels a
// caver had to go and find by hand every time a bay opened, and put
// away again by hand afterwards -- from a third panel, which is the
// hopping about (Nathan, 2026-09-10: "without having to hop back and
// forth between pallets").
//
// So opening a bay opens them and capturing puts them back. RESTORES,
// not hides: a caver who already had Feature Trace open was not asking
// for it to be closed by a section they happened to capture. In the
// ordinary case -- both shut before the bay -- restoring is hiding,
// which is what was asked for.
//
// THE STATE LIVES IN SETTINGS, not in a variable. The bay is opened
// from one script context (SectionBay, reached from Sketch Scans or
// Cross Section) and captured from another (SectionCapture's own
// action), and a module-level variable in one of them is not visible to
// the other. A one-line setting is.
// ---------------------------------------------------------------------

/** The docks a bay wants open, by objectName -- which is how a dock is
 *  found from a context that does not own it. */
CsPanel.TRACING_DOCKS = ["CaveSurveyFeatureTraceDock",
    "CaveSurveySymbolPaletteDock"];

/** Where the pre-bay visibility is remembered between contexts. */
CsPanel.SETTING_RESTORE = "CaveSurvey/BayDockRestore";

/** One dock by objectName, or null. */
CsPanel.dockNamed = function(name) {
    try {
        var win = RMainWindowQt.getMainWindow();
        var dock = win.findChild(name);
...
/**
 * Opens the tracing panels, remembering which were already open.
 *
 * Safe to call twice: a second call with a snapshot already stored
 * leaves the FIRST one alone, so a bay opened over a bay cannot lose
 * the caver's real starting state.
 */
CsPanel.openTracingDocks = function() {
    var already = "";
    try {
        already = String(RSettings.getStringValue(
            CsPanel.SETTING_RESTORE, ""));
    } catch (eRead) {
        already = "";
    }
    var shown = [];
    for (var i = 0; i < CsPanel.TRACING_DOCKS.length; i++) {
        var name = CsPanel.TRACING_DOCKS[i];
        var dock = CsPanel.dockNamed(name);
        if (dock === null) {
            continue;
        }
        try {
            if (already === "" && dock.visible === true) {
                shown.push(name);
            }
            dock.visible = true;
        } catch (eShow) {
    ...
    if (already === "") {
        try {
            // The empty string means "nothing was open", which is why
            // the marker below is written even when the list is empty:
            // an absent setting has to mean "no bay is running".
            RSettings.setValue(CsPanel.SETTING_RESTORE,
                "|" + shown.join(","));
        } catch (eWrite) {
        }
    }
...
/** Puts the tracing panels back the way they were before the bay. */
CsPanel.restoreTracingDocks = function() {
    var stored = "";
    try {
        stored = String(RSettings.getStringValue(
            CsPanel.SETTING_RESTORE, ""));
    } catch (eRead) {
        stored = "";
    }
    if (stored === "") {
        return;   // no bay opened them: nothing of ours to put back
    }
    var wereOpen = {};
    var names = stored.replace(/^\|/, "").split(",");
    for (var n = 0; n < names.length; n++) {
        if (names[n] !== "") {
            wereOpen[names[n]] = true;
        }
    }
    for (var i = 0; i < CsPanel.TRACING_DOCKS.length; i++) {
        var name = CsPanel.TRACING_DOCKS[i];
        ...
        try {
            dock.visible = (wereOpen[name] === true);
        } catch (eSet) {
        }
    }
    try {
