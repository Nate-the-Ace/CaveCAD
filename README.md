...
| Tool | Command | What it does |
| --- | --- | --- |
| Draw | `draw` | ONE panel for putting marks on the map: **Trace** on the left and **Symbols** on the right, two foldable columns of one dock. Drawing a cave is one job done with one hand -- a wall, then the ledge below it, then the stalactite beside that -- and it used to be two docks, so a caver was choosing which PANEL to look at before choosing what to draw, and on a laptop only one of them fitted beside the drawing. Columns and not tabs, deliberately: tracing a passage means reaching for a wall and a symbol in the same breath, so both stay in view, and side by side each gets the dock's full height -- the dimension a wall of tiles actually needs. Fold a column by its header and it shrinks to a strip, handing its width to the other; what you fold is remembered. Left is tracing and right is symbols, always, with no reorder to learn. `ft` and `sym` both open it, because a caver who has typed them for a year should not have to learn that they now mean the same door; `featuretrace` and `symbolpalette` still type -- they open this panel with that section unfolded. Neither half builds a dock of its own any more: each keeps its widgets in one module-level record, so a second copy of the body would look right and do nothing when clicked. |
